...
public class Server {
  ...
  private Map<String,ArrayDeque> _roundRobinListeners;
  ...
  public Server() {
    ...
    _roundRobinListeners = new HashMap();
  }
  ...
  protected void receive( Command c, Map h, String b, Authenticatable y ) {
    ...
    try {
      ...
      if (c == Command.COMMIT) {
        ...
        //System.out.println("\nSTOMP server received a command [" + c.toString() + "] for destination [" + destination + "]");
        ...
        if (c == Command.SEND) {
          if (y instanceof IntraVMClient ||
            ...
            synchronized( _listeners ) {
                ...
                ArrayDeque ad = _roundRobinListeners.get(destination);
                if ((ad != null) && (l != null)) {
                    Listener sh = (Listener) ad.pollFirst();
                    if ((sh != null) && (l.contains(sh))) {
                        try {
                      
                            sh.message( h, b );
                        
                        } catch (Exception e) {
                          System.out.println("\nSTOMP server encountered an error for command [" + c.toString() + "] for destination [" + destination + "] with listener [" + sh + "]");
                        }
                    ad.addLast(sh);
          ...
          if (y instanceof IntraVMClient ||
            ...
            synchronized (_listeners ) {
              ...
              if (l == null) {
                ...
                _roundRobinListeners.put( destination, new ArrayDeque<>() ); 
              }
              if (!l.contains(y)) {
                  l.add( y );
                  _roundRobinListeners.get(destination).addLast(y);
              }
            }
          } else {
            Map error_headers = new HashMap();
          ...
          synchronized (_listeners) {
            ...
            for (Iterator i=_roundRobinListeners.values().iterator(); i.hasNext(); ) {
              ArrayDeque ad = (ArrayDeque)i.next();
              ad.remove( y );
            }
          }
        }
