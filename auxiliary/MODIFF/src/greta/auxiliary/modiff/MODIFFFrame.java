...
import static greta.auxiliary.modiff.MODIFF.LOGGER;
 ...
 * @author Michele
 */
public class MODIFFFrame extends javax.swing.JFrame implements AUEmitter, BAPFrameEmitter, CharacterDependent, FeedbackPerformer {

    /**
     * Creates new form MODIFFFrame
     */
    ...
    public final static String BLINK_AU = "AU45_r";
     ...
     private String MODIFF_launcher_path  = "Common\\Data\\MODIFF\\bat_run_modiff.bat";
//    private String MODIFF_launcher_path  = "E:\\2024-2026_ISIR\\NetBeansProjects\\greta_20240917_demo_v2_MODIFF\\greta\\bin\\Common\\Data\\MODIFF\\bat_run_modiff.bat";
    // private String MODIFF_launcher_path  = "DIR";
//                                           "Common\\Data\\MODIFF\\bat_run_modiff.bat"
    //private String MODIFF_killer_path = "";
    private Process server_process;
    //private Thread server_shutdownHook;    
    
    public MODIFFFrame(CharacterManager cm) {
        initComponents();
        server = new Server();
        this.cm = cm;
        cm.add(this);
        ...
        //
        // Activate/deactivate this block to manually activate modiff python code
        //
        
//        try {
//            init_MODIFFF_server("greta.auxiliary.modiff.MODIFFFrame.java");
//        } catch (InterruptedException ex) {
//            Logger.getLogger(MODIFFFrame.class.getName()).log(Level.SEVERE, null, ex);
//        }
    ...
    public void init_MODIFFF_server(String this_file_path) throws InterruptedException
    {
        System.out.println(this_file_path + ".init_MODIFF_server(): MODIFF, initializing python environment...");
        try{
            server_process = new ProcessBuilder(MODIFF_launcher_path).redirectErrorStream(true).redirectOutput(ProcessBuilder.Redirect.INHERIT).start();
            // server_shutdownHook = new shutdownHook(server_process, MODIFF_killer_path);
            Runtime.getRuntime().addShutdownHook(new Thread(new ShutdownHookClass()));
        } catch (IOException ex2){
            Logger.getLogger(MODIFFFrame.class.getName()).log(Level.SEVERE, null, ex2);
        }
    }
    
    public class ShutdownHookClass implements Runnable {

        @Override
        public void run() {
            System.out.println("\nTrying to kill MODIFF process...");
    ...
    private void connexionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_connexionActionPerformed
        ...
        if (connexion.isSelected()) {
            ...
            try {
                ...
                System.out.println("greta.auxiliary.modiff.MODIFFFrame:" + server.port + "   " + server.address);
            ...
            System.out.println("greta.auxiliary.modiff.MODIFFFrame.connexionActionPerformed() UNCHECKED");
            try {
                // r2.join();
                server.setStop(true);
    ...
    private double readAUDataCol(String key, String[] cols, Map<String, AUFeature> set) {
     ...
     public void readDataLine(String data) {
      ...
      public void copy(MODIFFFrame f) {
      ...
      public int loadMODIFF(String line_data) {
    ...
    private void send(int length) {
        ...
        for (AUAPFrame frame : au_frames) {
            ...
            for (AUPerformer performer : au_perfomers) {
                ...
                if (this.parent != null && this.IsRunning == Boolean.TRUE){
                    ...
                    performer.performAUAPFrame(frame, id);
                    System.out.println(" LE ADD ANIMATION EST PASSEee 3");
                }
        ...
        for (BAPFramePerformer performer : bap_perfomers) {
            //System.out.println("[INFO]:greta.auxiliary.modiff.AUParserFilesReader.send()");
