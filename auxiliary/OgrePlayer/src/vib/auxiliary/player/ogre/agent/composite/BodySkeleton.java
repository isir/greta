...
package vib.auxiliary.player.ogre.agent.composite;
...
public class BodySkeleton {
    ...
    private vib.core.util.math.Quaternion[][] originalderivedOrientation; //(parents orientation)
    ...
    public BodySkeleton(SkeletonInstance skel) {
        ...
        originalderivedOrientation = new vib.core.util.math.Quaternion[numBones][2];
        ...
        for (int i = 0; i < numBones; i++) {
            ...
            try {
                ...
                originalderivedOrientation[i][0] = new vib.core.util.math.Quaternion();
                }
            } catch (Throwable t) {
                originalderivedOrientation[i][0] = new vib.core.util.math.Quaternion();
            }
...
//            }
    ...
    private void applyValues() {
        Ogre.callSync(new OgreThread.Callback() {
            ...
            public void run() {
                ...
                for (int i = 0; i < numBones; i++) {
                    if (mask[i]) {
                        vib.core.util.math.Quaternion q = eulerXYZToQuaternion(values[i][0], values[i][1], values[i][2]);
    ...
    private vib.core.util.math.Quaternion eulerXYZToQuaternion(double x, double y, double z) {
        vib.core.util.math.Quaternion qz = new vib.core.util.math.Quaternion(new Vec3d(0, 0, 1), (float) z);
        vib.core.util.math.Quaternion qy = new vib.core.util.math.Quaternion(new Vec3d(0, 1, 0), (float) y);
        vib.core.util.math.Quaternion qx = new vib.core.util.math.Quaternion(new Vec3d(1, 0, 0), (float) x);

        return mult(mult(qz, qy), qx);
    }

    private vib.core.util.math.Quaternion mult(vib.core.util.math.Quaternion q1, vib.core.util.math.Quaternion q2) {
        return vib.core.util.math.Quaternion.multiplication(q1, q2);
    }
