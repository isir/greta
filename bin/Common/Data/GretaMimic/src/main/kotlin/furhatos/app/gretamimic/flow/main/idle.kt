...
val Idle: State = state {
    ...
    onEntry {
        ...
        furhat.voice = Voice(language = Language.FRENCH, pitch = "medium", rate = 1.0)
        ...
        val rotationCallback: (Double?, Double?, Double?, Double?, String) -> Unit = { x, y, z, frameNum, event ->
            ...
            pitch = y
            yaw = z
            //println("$roll $pitch $yaw at $frameNum")
    ...
    defineGesture("RotateHead") {
        if (roll!=null && pitch!=null && yaw!=null) {
            frame(0.05, persist = true) {
                BasicParams.NECK_TILT to roll*3 // vetical head mouvement (head node)
                BasicParams.NECK_PAN to pitch*3 // horizontal (head shake)
                BasicParams.NECK_ROLL to yaw*3 // head tilt
            }
        }
    }
