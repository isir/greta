...
import numpy as np
from Users.UserMI__parra import UserMI
import torch
class DialogueEnvMI(gym.Env):
    ...
    def __init__(self,n_parra=5, render_mode=None):
        ...
        high = np.array(
            [
                1,
            ]*( self.n_agent_da+ self.n_user_da+1+4),
            dtype=np.int32,
        )
        low = np.array(
            [
    ...
    def _get_obs(self):
        ...
        master_ob = np.array([[self.user.context[i]/30,self.user.rapport[i]/30,self.user.perspective[i]/30,self.turn_id[i]/40] for i in range(self.n_parra)])
        ob = np.concatenate([master_ob,user_da, agent_da, np.array([[self.turn_id[i] / 40] for i in range(self.n_parra)])],
                            axis=1)
        return master_ob,ob
