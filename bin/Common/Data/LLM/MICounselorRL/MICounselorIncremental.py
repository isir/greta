...
def ask(question,messages=None,messages_online=None):
    ...
    if model == 'Local':
        return ask_local_chunk(question,language,subject,system_prompt, messages)
    else:
...
def ask_online_chunk(question,language,subject,system_prompt,messages=None):
    ...
    subject_prompt = open(os.path.join(os.path.dirname(__file__), "prompts/" + language + "_" + subject + ".txt"),
                          "r").read()
    if language == 'FR':
