...
/**
 * This class contains informations about face Signals.
 *
 * @author Andre-Marie Pez
 * @author Radoslaw Niewiadomski
 */
public class FaceSignal extends ParametricSignal {

    //from ParametricSignal
    //private String category- type au or faceexp
    //private String reference - experession name or au number
    ...
    /**
     * Construct a new {@code FaceSignal}
     *
     * @param id the identifier of this {@code FaceSignal}
     */
    public FaceSignal(String id) {
        super();
        init();
    ...
    public TimeMarker getConcretizedTimeMarker(String name) {
        TimeMarker temp = getTimeMarker(name);
        if ((temp != null) && (temp.isConcretized())) {
            return temp;
        }
        return null;
    }
            ...
            if ((!(start.isConcretized())) && (!(end.isConcretized())) && (!(decay.isConcretized())) && (!(sustain.isConcretized())) && (!(attack.isConcretized()))) {
                ...
                if ((start.isConcretized() ? 1 : 0) + (end.isConcretized() ? 1 : 0) + (decay.isConcretized() ? 1 : 0) + (sustain.isConcretized() ? 1 : 0) + (attack.isConcretized() ? 1 : 0) >= 2) {
                    ...
                    if ((!attack.isConcretized()) && (decay.isConcretized()) && sustain.isConcretized()) {
                        ...
                        start.setValue(decay.getValue() - duration * (3.0d / 5.0d)); //??? AM: 2/5 will match the others equations
                    }
                    if ((!attack.isConcretized()) && (!decay.isConcretized()) && sustain.isConcretized() && end.isConcretized()) {
                    ...
                    if ((start.isConcretized() ? 1 : 0) + (end.isConcretized() ? 1 : 0) + (decay.isConcretized() ? 1 : 0) + (sustain.isConcretized() ? 1 : 0) + (attack.isConcretized() ? 1 : 0) == 1) {
                        ...
                        if (attack.isConcretized()) {
                            start.setValue(attack.getValue() / 2.0);// ??? AM: no sens
                        }
                        if ((!attack.isConcretized()) && decay.isConcretized()) {
                            start.setValue(decay.getValue() / 2.0);// ??? AM: no sens
                            attack.setValue(decay.getValue() - decay.getValue() / 3.0);// ??? AM: no sens
                        }
                        if ((!attack.isConcretized()) && (!decay.isConcretized()) && sustain.isConcretized()) {
                            start.setValue(sustain.getValue() / 2.0);// ??? AM: no sens
                            attack.setValue(sustain.getValue() - sustain.getValue() / 3.0);// ??? AM: no sens
                            decay.setValue(sustain.getValue() - sustain.getValue() / 4.0);// ??? AM: no sens
                        }
                        if ((!attack.isConcretized()) && (!sustain.isConcretized()) && (!decay.isConcretized()) && end.isConcretized()) {
                            start.setValue(end.getValue() / 2.0);// ??? AM: no sens
                            attack.setValue(end.getValue() - end.getValue() / 3.0);// ??? AM: no sens
                            decay.setValue(end.getValue() - end.getValue() / 4.0);// ??? AM: no sens
                            sustain.setValue(end.getValue() - end.getValue() / 5.0);// ??? AM: no sens
                        }
                    }//end of if
                }//end of else
            }//end of else
